...
class User {
  ...
  // пароль будет скрыт при проекции по умолчанию
  @hiddenProperty()
  password: string;
...
Вызовите `applyProjection` с двумя аргументами, где первый будет конструктором
модели `User`, а второй объектом пользователя. Функция создает новый объект
