...
```typescript
import {
  embedded,      // определение вложенной модели
...
class User {
  ...
  @embedded(() => Profile) // определение вложенной модели
  profile: Profile;
...
**Важно:** В `@embedded(() => Profile)` используется функция-фабрика
...
помеченные `@embedded`, и применяет к ним правила проекции, определенные
...
Структурные декораторы:  
*(только для свойств экземпляра)*

* `@embedded(modelFactory: () => Constructor)`:  
